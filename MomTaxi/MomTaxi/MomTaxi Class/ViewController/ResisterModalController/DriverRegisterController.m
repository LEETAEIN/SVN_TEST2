...
@synthesize mPasswordCkeckTextField;
@synthesize mToolbar;
...
- (void)setAccessorryView
{
    ...
    [self.mPasswordCkeckTextField setInputAccessoryView:self.mToolbar];
    [self.mPasswordTextField setInputAccessoryView:self.mToolbar];
...
- (IBAction)toolbarAction:(UIBarButtonItem*)sender
{
    ...
    [self.mPasswordCkeckTextField resignFirstResponder];
...
- (IBAction)registerAction:(id)sender
{
    ...
    {
        alertView = [[UIAlertView alloc] initWithTitle:@"알림" message:@"휴대전화 번호를 확인하여 주세요." delegate:self cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
    
    if([self.mPasswordTextField.text length] < 4)
    {
        alertView = [[UIAlertView alloc] initWithTitle:@"알림" message:@"비밀번호는 4자리 이상 입력하셔야 합니다." delegate:self cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
    
    if([self.mPasswordCkeckTextField.text length] < 4)
    {
        alertView = [[UIAlertView alloc] initWithTitle:@"알림" message:@"비밀번호는 4자리 이상 입력하셔야 합니다." delegate:self cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    ...
    if([self.mPasswordCkeckTextField.text isEqualToString:self.mPasswordTextField.text] == NO)
    {
        alertView = [[UIAlertView alloc] initWithTitle:@"알림" message:@"비밀번호를 확인하여 주세요." delegate:self cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
    ...
    [self writePassword:self.mPasswordCkeckTextField.text];
...
{
    NSData *data = [[NSData alloc] initWithData:[self.mPasswordCkeckTextField.text dataUsingEncoding:NSUTF8StringEncoding]];
	...
	NSString *optionPath = [docDirPath objectAtIndex:0x00];
    NSString *fileName = [NSString stringWithFormat:@"data.dat"];
	NSString *dstPath = [optionPath stringByAppendingPathComponent:fileName];
    
	[object writeToFile:dstPath atomically:NO encoding:NSUTF8StringEncoding error:nil];
}
...
{    
    ...
    else if(textField == self.mPasswordCkeckTextField)
    {
        [self.mScrollView setContentOffset:CGPointMake(0.0, 380.0f) animated:YES];
    }
    ...
    {
        [self.mScrollView setContentOffset:CGPointMake(0.0, 450.0f) animated:YES];
    }
    else if(textField == self.mCompanyTextField)
    {
        [self.mScrollView setContentOffset:CGPointMake(0.0, 520.0f) animated:YES];
    }
...
{
    NSString *apiElement = gDefineAPI[valueRegister];
    ...
    [body appendData:[[NSString stringWithFormat:@"\r\n--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
	[body appendData:[@"Content-Disposition: form-data; name=\"pv_str\";type=\"text\"\r\n\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
#if 0
	[body appendData:[self.mCertifyTextField.text dataUsingEncoding:NSUTF8StringEncoding]];
#else
...
{
    NSString *apiElement = gDefineAPI[valueChangeMode];
    NSString *apiAddress = [apiElement stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
	NSString *boundary = @"----1010101010";
	NSString *contentType = [NSString stringWithFormat:@"multipart/form-data; boundary=%@", boundary];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    NSLog(@"DriverRegisterController == %@", dictionary);
